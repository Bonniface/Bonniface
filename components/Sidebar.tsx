...
const Sidebar: React.FC<SidebarProps> = ({ user, currentView, onChangeView, onLogout, isOpen, onClose }) => {
  ...
  return (
    <>
      <div className={`fixed inset-0 bg-navy-950/80 backdrop-blur-sm z-40 lg:hidden transition-opacity duration-300 ${isOpen ? 'opacity-100' : 'opacity-0 pointer-events-none'}`} onClick={onClose} />
      <div className={`fixed top-0 left-0 bottom-0 z-50 w-64 bg-white dark:bg-navy-950 border-r border-slate-200 dark:border-navy-800 flex flex-col p-4 transition-transform duration-300 ease-in-out lg:translate-x-0 ${isOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        ...
        <div className="flex-1 space-y-2 overflow-y-auto">
          ...
          <NavItem view="MESSAGES" icon={MessageSquare} label="Messages" badgeCount={3} />
          <NavItem view="BOOKINGS" icon={Calendar} label="Bookings" />
          
          <div className="px-2 mt-6 mb-2 text-xs font-semibold text-slate-400 dark:text-slate-500 uppercase tracking-wider">Management</div>
          <NavItem view="PROJECTS" icon={FolderKanban} label="Projects" />

          {!isAdmin && (
            <><NavItem view="NEW_PROJECT" icon={PlusCircle} label="Start Project" /><NavItem view="BILLING" icon={CreditCard} label="Billing" /></>
          )}

          {isAdmin && <NavItem view="INVOICES" icon={Receipt} label="Invoices" />}
        </div>

        <div className="pt-6 border-t border-slate-200 dark:border-navy-800 space-y-2">
          <NavItem view="SETTINGS" icon={Settings} label="Settings" />
          <button onClick={onLogout} className="w-full flex items-center gap-3 px-4 py-3 rounded-xl text-red-500 hover:bg-red-50 dark:hover:bg-navy-800/50 transition-colors duration-200">
